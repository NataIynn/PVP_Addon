...
//  heavy load)
// Note: we use [SyncVar_] a few times to make room for SyncLists in the addon
//  system. (UNET only allows 32 SyncVars+SyncLists) 
using UnityEngine;
...
public partial class Player : Entity {
    ...
    [Header("Attributes")]
    [SyncVar_, SerializeField] public int strength = 0;
    [SyncVar_, SerializeField] public int intelligence = 0;

    [Header("Experience")] // note: int is not enough (can have > 2 mil. easily)
    [SyncVar_, SerializeField] long _experience = 0;
    ...
    [SyncVar_] public long skillExperience = 0;
    ...
    [SerializeField, SyncVar_] long _gold = 0;
    ...
    [Header("Trading")]
    [SyncVar_, HideInInspector] public string tradeRequestFrom = "";
    [SyncVar_, HideInInspector] public bool tradeOfferLocked = false;
    [SyncVar_, HideInInspector] public bool tradeOfferAccepted = false;
    [SyncVar_, HideInInspector] public long tradeOfferGold = 0;
    ...
    [SyncVar_, HideInInspector] public string guild = "";
    //[SyncVar, HideInInspector] public string guildNotice = "";
    [SyncVar_, HideInInspector] public string guildInviteFrom = "";
    ...
    [SyncVar_, HideInInspector] public float nextRiskyActionTime = 0;
    ...
    public override void Recover() {
        ...
        if (enabled && health > 0) {
            ...
            // addon system hooks
            Utils.InvokeMany(typeof(Player), this, "Recover_");
        }
    }
    ...
    [Server]
    public void BroadcastGuildChanges(string guild, List<GuildMember> members) {
        // save in database
        Database.SaveGuild(guild, members);
    ...
    public void BroadcastOnlineStatusToGuild(bool online) {
        ...
        if (index != -1) {
            ...
            BroadcastGuildChanges(guild, guildMembers.ToList());
        }
    }
    ...
    public void CmdGuildInviteAccept() {
        ...
        if (sender != null) {
            if (sender.CanGuildInvite(this)) {
                ...
                BroadcastGuildChanges(guild, sender.guildMembers.ToList());
    ...
    public void CmdSetGuildNotice(string notice) {
        ...
        if (health > 0 && notice.Length < guildNoticeMaxLength &&
            Time.time >= nextRiskyActionTime) {
            // TODO when we can use another syncvar again later
            print("TODO set notice to: " + notice);
    ...
    public void OnDragAndDrop(string fromType, int from, string toType, int to) {
        // call OnDragAndDrop_From_To dynamically
        print("OnDragAndDrop from: " + fromType + " " + from + " to: " + toType + " " + to);
        SendMessage("OnDragAndDrop_" + fromType + "_" + toType, new int[]{from, to},
